...
export class BusinessRolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const isBusinessCreationRoute = 
      !businessId && 
      request.method === 'POST' && 
      request.path.endsWith('/businesses');
    
    if (isBusinessCreationRoute && requireSuperAdmin) {
      if (user.githubId) {
        return true;
      }
      throw new ForbiddenException('Only owner withGitHub account can create a business');
    }

    if (!businessId && !isBusinessCreationRoute) {
